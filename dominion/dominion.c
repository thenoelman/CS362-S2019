...
int cardEffect_adventurer(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int temphand[MAX_HAND], int z)
{
	while (drawntreasure < 2) {
		if (state->deckCount[currentPlayer] < 1) {//if the deck is empty we need to shuffle discard and add to deck
			shuffle(currentPlayer, state);
		...
		drawCard(currentPlayer, state);
		cardDrawn = cardDrawnFunction(state, currentPlayer, state->hand[currentPlayer], state->handCount[currentPlayer] - 1);
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
			drawntreasure++;
		else {
			temphand[z] = cardDrawn;
...
int cardEffect_smithy(int i, int currentPlayer, struct gameState *state, int handPos)
{
	//+3 Cards
	for (i = 0; i < 3; i++)
	{
