...
{
	if (card_played == council_room)
	{
		printf("Test 3a: Test handCount is + 1 card\n");
		...
		{
			printf("----- TEST FAILED - other player's handCount has not increased by one.\n");
			*testFailed++;
		}
	}
	else
	...
	if (card_played == council_room)
	{
		printf("Test 3b: Test deckCount has decreased by one\n");
		...
		if (testG.deckCount[thisPlayer + 1] == G.deckCount[thisPlayer + 1] - 1)
		{
			printf("+++++ TEST PASSED\n");
			*testPassed++;
		}
